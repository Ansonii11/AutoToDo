...
export class TaskViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "autotodo-view";
  ...
  constructor(
    ...
    this._taskService.onReminder((task) => {
      ...
      vscode.window
        .showInformationMessage(message, MESSAGES.VIEW_TASK)
        .then((selection) => {
          if (selection === MESSAGES.VIEW_TASK) {
            vscode.commands.executeCommand("autotodo-view.focus");
          }
        });
    });
  ...
  public async resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage((data) =>
      this._webviewHandler.handleMessage(data, webviewView)
    );
  }

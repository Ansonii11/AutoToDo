...
export class TaskWebview {
  public static getHtml(
    ...
    const flatpickrCssUri = webview.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, "media", "flatpickr.min.css")
    );
    const flatpickrDarkCssUri = webview.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, "media", "flatpickr_dark.css")
    );
    const flatpickrJsUri = webview.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, "media", "flatpickr.min.js")
    );
    ...
    return `<!DOCTYPE html>
            ...
            <body>
                ...
                <div id="priorityPopover" class="premium-popover">
                    ...
                    <div class="popover-divider"></div>
                    <div class="popover-item" onclick="toggleAutoPriorityPicker(event)">
                        <i class="codicon codicon-sparkle" style="color: #AF52DE"></i>
                        <span>${UI.AUTO_PRIORITY}</span>
                    </div>
                </div>

                <div id="autoPriorityPopover" class="premium-popover auto-priority-popover">
                    <div class="popover-section-title">${UI.AUTO_PRIORITY}</div>
                    
                    <div class="auto-priority-row">
                        <div class="row-label-group">
                            <span class="row-label">${UI.IMPACT}</span>
                            <span id="impactValue" class="value-badge">3</span>
                        </div>
                        <input type="range" id="impactSlider" min="1" max="5" step="1" value="3" oninput="document.getElementById('impactValue').innerText = this.value">
                    </div>

                    <div class="auto-priority-row">
                        <div class="row-label-group">
                            <span class="row-label">${UI.DIFFICULTY}</span>
                            <span id="difficultyValue" class="value-badge">3</span>
                        </div>
                        <input type="range" id="difficultySlider" min="1" max="5" step="1" value="3" oninput="document.getElementById('difficultyValue').innerText = this.value">
                    </div>

                    <div class="auto-priority-row">
                        <div class="row-label-group">
                            <span class="row-label">${UI.TIME_REQUIRED}</span>
                        </div>
                        <input type="number" id="timeRequiredInput" class="custom-date-input" placeholder="Hours..." min="0.1" step="0.5" value="1">
                    </div>

                    <div class="auto-priority-row">
                        <div class="row-label-group">
                            <span class="row-label">${UI.DATE}</span>
                        </div>
                        <div class="custom-date-wrapper">
                            <i class="codicon codicon-calendar"></i>
                            <input type="text" id="autoPriorityDateInput" class="custom-date-input" placeholder="Select due date...">
                        </div>
                    </div>

                    <div class="popover-footer">
                        <button class="modal-btn primary-btn" onclick="applyAutoPriority()" style="width: 100%">

...
export function activate(context: vscode.ExtensionContext) {
  ...
  statusBarItem.command = "autotodo-view.focus";
  context.subscriptions.push(statusBarItem);

  const updateStatusBar = (tasks: TodoItem[]) => {
    const total = tasks.length;
    const mustCount = tasks.filter(
    ...
    if (total === 0) {
      ...
      statusBarItem.tooltip =
        MESSAGES.TASKS_PENDING.replace("{0}", mustCount.toString()) +
        "\n" +
  ...
  context.subscriptions.push(
    taskService.onTasksChanged((tasks) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("autotodo.refresh", () => {
      provider.refresh();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("autotodo.openFull", () => {
      ...
      const updatePanel = async () => {
        ...
        panel.webview.html = TaskWebview.getHtml(
          panel.webview,

function toggleAutoPriorityPicker(e) {
  if (e) e.stopPropagation();
  const pop = document.getElementById("autoPriorityPopover");
  if (!pop) return;

  const isShow = pop.classList.contains("show");
  // Don't close priorityPopover yet if we want to go back, but for simplicity let's close all
...
function applyAutoPriority() {
  const impact = parseInt(document.getElementById("impactSlider").value);
  const difficulty = parseInt(
    document.getElementById("difficultySlider").value
  );
  const timeRequired =
    parseFloat(document.getElementById("timeRequiredInput").value) || 1;

  let dueDate =
    autoPriorityPicker && autoPriorityPicker.selectedDates[0]
      ? autoPriorityPicker.selectedDates[0].getTime()
      : modalTaskId
      ? modalDueDate
      : currentPremiumDate;
  if (!dueDate) {
    // Default to 1 week from now if no date is set
    dueDate = Date.now() + 7 * 24 * 60 * 60 * 1000;
  }

  const now = Date.now();
  const timeRemainingHours = Math.max((dueDate - now) / (1000 * 60 * 60), 0.1);

  // Normalization:
  // Impact (1-5), Difficulty (1-5)
  // TimeRequired normalized to a workday (8h)
  // TimeRemaining normalized to a work week (40 business hours)
  const normTimeRequired = timeRequired / 8;
  const normTimeRemaining = timeRemainingHours / 40;

  const weights = { impact: 4, difficulty: 2, time_required: 1.5 };
  const score =
    (impact * weights.impact +
      difficulty * weights.difficulty +
      normTimeRequired * weights.time_required) /
    Math.max(normTimeRemaining, 0.1);

  let priority = "Wont";
  if (score >= 25) priority = "Must";
  else if (score >= 12) priority = "Should";
  else if (score >= 5) priority = "Could";

  if (autoPriorityPicker && autoPriorityPicker.selectedDates[0]) {
    const selDate = autoPriorityPicker.selectedDates[0].getTime();
    if (modalTaskId) {
      modalDueDate = selDate;
      vscode.postMessage({
        type: "updateDueDate",
        id: modalTaskId,
        dueDate: selDate,
      ...
      setPremiumDate(new Date(selDate).toISOString().split("T")[0], true);
    }
  }

  setPremiumPriority(priority);
  closeAllPopovers();
}
...
function openTaskModal(taskId) {
  ...
  if (reminderPicker) {
    const r =
      task.reminders && task.reminders.length > 0 ? task.reminders[0].time : "";
...
function updateModalUI() {
  ...
  if (dateVal) {
    ...
    if (modalDueDate) {
      ...
      dateVal.querySelector("span").innerText = `${
        months[d.getMonth()]
      } ${d.getDate()}`;
  ...
  if (reminderVal) {
    ...
    if (modalReminders.length > 0) {
      ...
      if (isToday) {
        ...
        reminderVal.querySelector("span").innerText = `${d.getDate()} ${
          months[d.getMonth()]
        }, ${h}:${m}`;
      }
  ...
  // Priority
  if (priorityVal) {
    priorityVal.querySelector("span").innerText =
      modalPriority === "Wont" ? "Won't" : modalPriority;
  ...
  if (priorityIndicator) {
    ...
    priorityIndicator.className = `priority-indicator is-${modalStatus
      .toLowerCase()
      .replace(" ", "-")}`;
  }
...
function closeAllPopovers(keepEditingId = false) {
  document
    .querySelectorAll(".premium-popover, .popover")
...
function toggleConfigPopover(e) {
  ...
  if (!isShow) {
    ...
    if (popoverBtnList)
      popoverBtnList.classList.toggle("active", viewMode === "list");
    if (popoverBtnKanban)
      popoverBtnKanban.classList.toggle("active", viewMode === "kanban");
  }
}
...
function clearReminder() {
  ...
  if (reminderInput) {
    reminderInput.value = "";
    reminderInput.classList.remove(
      "date-color-today",
      "date-color-tomorrow",
      "date-color-soon",
      "date-color-weekend",
...
function setPremiumReminder(m, labelText, isCustom = false) {
  ...
  if (m === "tomorrow") {
    ...
    typeof m === "string" &&
    (m.includes("T") || m.includes(" ") || m.includes("-"))
  ) {
...
function setViewMode(mode) {
  ...
  if (popoverBtnList)
    popoverBtnList.classList.toggle("active", mode === "list");
  if (popoverBtnKanban)
    popoverBtnKanban.classList.toggle("active", mode === "kanban");
...
function setGroupBy(mode) {
  ...
  if (priorityCheck)
    priorityCheck.classList.toggle("hidden", mode !== "priority");
...
function showContextMenu(e, taskId) {
  ...
  if (task) {
    ...
    if (clearDateBtn)
      clearDateBtn.style.display = task.dueDate ? "flex" : "none";
    if (clearReminderBtn)
      clearReminderBtn.style.display =
        task.reminders && task.reminders.length > 0 ? "flex" : "none";
  }
  menu.hidden = false;
...
function updateStatusFromMenu(s) {
  if (activeTaskId)
    vscode.postMessage({ type: "updateStatus", id: activeTaskId, status: s });
  hideContextMenu();
}
...
function getPriorityIndicatorHtml(t, index) {
  ...
  const rankTag =
    index !== undefined && !isDone
      ? `<span class="priority-rank" style="color: ${pColor}">${
          index + 1
        }</span>`
      : "";
...
function getTaskBadgesHtml(t) {
  ...
  if (t.dueDate) {
    ...
    if (date.getTime() === now.getTime()) {
      label = "Today";
      colorClass = "date-color-today";
    } else if (date.getTime() === tomorrow.getTime()) {
      label = "Tomorrow";
      colorClass = "date-color-tomorrow";
    } else if (
      date.getTime() > tomorrow.getTime() &&
      date.getTime() <= in3Days.getTime()
    )
      colorClass = "date-color-soon";
...
function renderList(tasks) {
                    ...
                    ${globalSortedTasks
                      .map(
                        ...
                        <div class="list-task-row ${
                          t.status === "Done" ? "task-is-done" : ""
                        }" data-id="${t.id}">
                            ${getPriorityIndicatorHtml(t, taskRanks.get(t.id))}
                            <div class="card-content">
                                <div class="card-title" ondblclick="event.stopPropagation(); makeEditable(this, '${
                                  t.id
                                }')">${t.text}</div>
                                ${
                                  t.description
                                    ? `<div class="card-desc" onclick="event.stopPropagation(); openTaskModal('${t.id}')">${t.description}</div>`
                                    : `<div class="card-desc empty-desc" onclick="event.stopPropagation(); openTaskModal('${t.id}')">Add description...</div>`
                                }
                                ${getTaskBadgesHtml(t)}
                            </div>
                            <div class="card-more" onclick="showContextMenu(event, '${
                              t.id
                            }')">
  ...
  const groups =
    groupBy === "status"
      ? ["Todo", "Ready", "In Progress", "Testing", "Done"]
      : ["Must", "Should", "Could", "Wont"];
  const displaySortedTasks = [...tasks].sort((a, b) => {
    const valA =
      groupBy === "status"
        ? a.status
        : a.priority === "Won't"
    ...
    if (groupBy === "priority") {
      return taskRanks.get(a.id) - taskRanks.get(b.id);
    }
    return groups.indexOf(valA) - groups.indexOf(valB);
  });
  groups.forEach((group) => {
    const sTasks = displaySortedTasks.filter((t) => {
      const val =
        groupBy === "status"
          ? t.status
          : t.priority === "Won't"
    ...
    div.innerHTML = `
            <div class="list-section-header" onclick="toggleSection('${group}')">
                ...
                ${
                  groupBy === "status"
                    ? `<i class="codicon ${STATUS_ICONS[group]} status-icon-header"></i>`
                    : ""
                }
...
function renderKanban(tasks) {
  ...
  cols.forEach((col) => {
    ...
    const cTasks = tasks
      ...
      .sort((a, b) => taskRanks.get(a.id) - taskRanks.get(b.id));
    const pVal =
      groupBy === "status"
        ? "null"
        : col === "Wont"
    ...
    div.ondrop = (e) => {
      ...
      if (taskId) {
        if (groupBy === "status")
          vscode.postMessage({ type: "updateStatus", id: taskId, status: col });
        else if (groupBy === "priority")
          vscode.postMessage({
            type: "updatePriority",
            id: taskId,
            priority: col === "Wont" ? "Won't" : col,
          });
      }
    };
    div.innerHTML = `
            <div class="col-header">
                <div class="col-title-wrapper">
                    ${
                      groupBy === "status"
                        ? `<i class="codicon ${STATUS_ICONS[col]} status-icon-header"></i>`
                        : ""
                    }
                    <span>${col}</span>
                </div>
                ...
                ${cTasks
                  .map(
                    ...
                    <div class="task-card ${
                      ...
                      t.id
                    }'); this.classList.add('dragging')" ondragend="this.classList.remove('dragging')">
                        ${getPriorityIndicatorHtml(t, taskRanks.get(t.id))}
                        <div class="card-content">
                            <div class="card-title" ondblclick="event.stopPropagation(); makeEditable(this, '${
                              t.id
    ...
    if (newestTask && newestTask.text === "New task...") {
      setTimeout(() => {
        const titleEl = document.querySelector(
          `[data-id="${newestTask.id}"] .card-title`
        );
...
function makeEditable(el, id, field = "text") {
  ...
  if (isNew) oldText = "";
  el.innerHTML = `<textarea class="edit-input" id="active-edit" placeholder="${
  ...
  const save = () => {
    ...
    if (val !== (isNew ? "" : oldText)) {
      if (field === "text") {
        ...
        vscode.postMessage({ type: "updateDescription", id, description: val });
    } else el.innerText = isNew ? "New task..." : oldText;
...
document.addEventListener("click", (e) => {
  const isTrigger =
    e.target.closest(".popover-wrapper") ||
...
let datePicker, reminderPicker, autoPriorityPicker;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof flatpickr !== "undefined") {
    ...
    autoPriorityPicker = flatpickr("#autoPriorityDateInput", {
      disableMobile: true,
      dateFormat: "Y-m-d",
      onChange: function (selectedDates, dateStr) {
        // Date is handled in applyAutoPriority
      },
    });
...
window.toggleAutoPriorityPicker = toggleAutoPriorityPicker;
window.applyAutoPriority = applyAutoPriority;
